...
import { Github, ArrowUpRight, Calendar, Star, GitFork, Code2 } from "lucide-react";
...
export default function ProjectPage({ repos }: { repos: Repo[] }) {
  return (
    <Layout>
      ...
      <main className="py-25 mx-auto max-w-7xl px-6 lg:px-8">
            ...
            {repos.map((repo) => (
              <motion.li key={repo.html_url} variants={itemVariants} className="group w-full">
                <Link href={repo.html_url} target="_blank" className="block w-full">
                  ...
                  <article className="relative w-full aspect-video flex flex-col justify-between p-6 rounded-[2rem] bg-white/70 dark:bg-slate-900/50 backdrop-blur-md border border-slate-200 dark:border-slate-800 transition-all duration-500 hover:shadow-2xl hover:shadow-sky-500/10 dark:hover:shadow-sky-900/20 hover:-translate-y-2 group-hover:border-sky-500/50 overflow-hidden">
                    
                    {/* Top Section: Header */}
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                         <div className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 group-hover:bg-sky-500 group-hover:text-white transition-colors duration-300">
                           <Github className="w-5 h-5" />
                         </div>
                         <h2 className="text-lg md:text-xl font-bold text-slate-900 dark:text-white group-hover:text-sky-500 transition-colors truncate max-w-[150px]">
                           {repo.name}
                         </h2>
                      </div>
