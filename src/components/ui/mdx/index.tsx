...
// Library untuk zoom gambar (opsional, jalankan: npm install react-medium-image-zoom)
...
img: (props: any) => {
  ...
  return (
    <figure className="my-10 w-full max-w-3xl mx-auto flex flex-col items-center group">
      {/* Parent Utama harus RELATIVE dan memiliki ASPECT RATIO yang kuat.
         Gunakan min-h untuk memastikan kontainer tidak nol.
      */}
      <div className="relative w-full aspect-video min-h-[200px] md:min-h-[350px] rounded-3xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-lg bg-slate-100 dark:bg-slate-900">
        ...
        {/* Bungkus Zoom di LUAR Image, tapi pastikan Image tetap mengisi kontainer */}
        <Zoom transitionDuration={400}>
  ...
  h1: (props: any) => (
    <h1 className="text-4xl md:text-5xl font-black tracking-tighter mb-10 mt-4 
  ...
  h2: (props: any) => (
    <h2 className="group relative text-2xl md:text-3xl font-bold my-10 
  ...
  h3: (props: any) => (
    <h3 className="mt-10 mb-4 text-xl md:text-2xl font-bold text-slate-800 dark:text-slate-100 scroll-mt-32" {...props} />
  ),

  p: (props: any) => (
    <p className="text-base md:text-lg leading-relaxed text-slate-600 dark:text-slate-400 mb-6 font-normal" {...props} />
  ),

  a: (props: any) => {
    const isInternal = props.href?.startsWith('/');
    const content = (
      <span className="relative inline-block font-semibold text-sky-600 dark:text-sky-400 
    ...
    if (isInternal) {
      return <Link href={props.href} {...props}>{content}</Link>;
    }
    return <a target="_blank" rel="noopener noreferrer" {...props}>{content}</a>;
  },

  ul: (props: any) => (
    <ul className="mt-8 mb-10 space-y-4 list-none pl-0" {...props} />
  ),

  li: (props: any) => (
    <li className="group flex items-start gap-4 text-slate-600 dark:text-slate-400">
      <div className="flex-none pt-[0.65rem]">
        <div className="h-[3px] w-4 bg-slate-300 dark:bg-sky-500 
  ...
  blockquote: (props: any) => (
    <blockquote className="my-12 border-l-4 border-sky-500 bg-slate-50 dark:bg-slate-900/40 
